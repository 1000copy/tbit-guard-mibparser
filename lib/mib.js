    ...
    return ({
        ...
        GetObject: function (string, callback) {
            ...
            if (typeof string == 'string' && string.indexOf(".") > -1) {
                ...
                for (m = 0; m < M_len; m++) {
                    var Objects = Object.keys(this.Modules[Modules[m]]), O_len = Objects.length;
                    for (o = 0; o < O_len; o++) {
                        if (this.Modules[Modules[m]][Objects[o]][stringtype] == string && !found) {
                            found = true;
                            callback(this.Modules[Modules[m]][Objects[o]]);
                            break;
                        }
                        if (string.indexOf(this.Modules[Modules[m]][Objects[o]][stringtype]) > -1 && !found) {
                            var length = this.Modules[Modules[m]][Objects[o]][stringtype].split(".").length;
                            if (length >= MatchLength) {
                                MatchLength = length;
                                MatchObject = this.Modules[Modules[m]][Objects[o]];
                            }
                        }
                    }
        ...
        LoadMIBs: function () {
            ...
            this.Import("./RFC_BASE_MINIMUM//reco.mib");
        ...
        WriteToFile: function (fname) {
        	//__dirname +
        	fs.writeFile("./RFC_BASE_MINIMUM/"+ fname, JSON.stringify(this.Modules, null, 4), function (err) {
                if (err) {
                    console.log(err);
                } else {
        ...
        DecodeVarBinds: function (varbinds, callback) {
            ...
            switch (lib) {
                ...
                case 'snmp-native':
                    ...
                    /* replace with faster Object.keys();
                    varbinds.forEach(function (vb) {
                        ...
                        if (vb.OID.replace(cached.OID, "").split(".").length == cached.oid.split(".").length) {
        ...
        GetVarBindVal: function (vb, varbind, Object, Object1, lib) {
            ...
            switch (vb.typename) {
                ...
                case 'IpAddress':
                case 'OctetString':
                    if (Object.SYNTAX == "OCTET STRING" || Object.SYNTAX.indexOf('ddr') > -1 || Object.SYNTAX == 'DateAndTime') {
                        var buffstring = "";
                        var delimiter = "."
                        var buf = (lib == 'snmp-native') ? vb.valueRaw : vb.value;
                        var DT = (Object.SYNTAX == 'DateAndTime');
                        var Physical = (Object.SYNTAX.indexOf('PhysAddress') > -1);
                        if (Physical) {
